...
def generate_qe_input(atoms: Atoms, calculation: str = 'scf', ecutwfc: float = 30.0) -> str:
    ...
    control_block = "&CONTROL\n" + "\n".join(
        f"    {key} = {value}" for key, value in control_params.items()
    ) + "\n/\n"
    system_block = "&SYSTEM\n" + "\n".join(
        f"    {key} = {value}" for key, value in system_params.items()
    ) + "\n/\n"
    electrons_block = "&ELECTRONS\n" + "\n".join(
        f"    {key} = {value}" for key, value in electron_params.items()
    ) + "\n/\n"
    ...
    for vector in atoms.cell:
        cell_params_lines.append(
            f"  {vector[0]:16.9f} {vector[1]:16.9f} {vector[2]:16.9f}"
        )
    ...
    for symbol, pos in zip(atoms.get_chemical_symbols(), positions, strict=True):
        atomic_positions_lines.append(
            f"  {symbol:<4} {pos[0]:16.9f} {pos[1]:16.9f} {pos[2]:16.9f}"
        )
