...
async def _refine_spec_async() -> None:
    ...
    if not spec_path.exists():
        console.print("[red]❌ ALL_SPEC.md not found. Run 'copy dev_documents/templates/ALL_SPEC.md dev_documents/' first.[/red]")
        raise typer.Exit(1)
...
"""
        for f in structured_spec.features:
            md_content += f"### {f.name} ({f.priority})\n{f.description}\n\n**Acceptance Criteria:**\n"
            md_content += "\n".join(f"- {ac}" for ac in f.acceptance_criteria) + "\n\n"

        md_content += "## Technical Constraints\n"
        md_content += "\n".join(f"- **{c.category}**: {c.description}" for c in structured_spec.constraints)
