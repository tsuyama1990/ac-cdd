ご提案いただいた「サイクル分割型・契約駆動開発（AC-CDD: AI-Native Cycle-Based Contract-Driven Development）」に基づき、あなたの構想と私の改善案（マージ戦略、Pydantic契約、厳格監査ループ）を完全に統合した詳細な設計書を作成しました。

この `DESIGN_FLOW.md` は、プロジェクトの「憲法」であると同時に、今後JulesやGeminiに指示を出す際の「ゴールデン・コンテキスト」として機能します。これをプロジェクトのルートに配置してください。
...
1.  **契約絶対主義 (Contract is King)**
    [cite_start]自然言語の仕様書は曖昧である。`Pydantic` スキーマによって定義された入出力構造（Contracts）のみを「実行可能な正解（Single Source of Truth）」とし、すべての実装はこの契約に従わなければならない [cite: 12, 14, 17]。
...
3.  **サイクル分割と自己修復 (Cyclic Self-Healing)**
    [cite_start]開発を管理可能な小さなサイクルに分割し、各サイクル内で「実装→テスト→監査→修正」のループを自律的に回すことで、人間の介入を承認プロセスのみに限定する [cite: 72, 85, 107]。
